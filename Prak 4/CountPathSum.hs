...
countPathSum p num
    | isTreeEmpty p = 0
    | isBiner p = (countPathSum (left p) (num - (akar p))) + (countPathSum (right p) (num - (akar p)))
    | isUnerLeft p = if (num - (akar p) == 0) then 1 + (countPathSum (left p) (num - (akar p)))
                        else 0 + (countPathSum(left p) (num - (akar p)))
    | isUnerRight p = if (num - (akar p) == 0) then 1 + (countPathSum (right p) (num - (akar p)))
                        else 0 + (countPathSum (right p) (num - (akar p)))
    | isOneElmt p= if (num - (akar p)) == 0 then 1 
                    else 0
{-
Contoh 1:
Pohon:

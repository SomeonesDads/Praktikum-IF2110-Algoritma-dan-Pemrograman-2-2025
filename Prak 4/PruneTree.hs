...
pruneTree p d
    ...
    | (d > 1) && isOneElmt p = makeBinTree (akar p) Empty Empty 
    | (d > 1) && isBiner p = makeBinTree (akar p) (pruneTree (left p) (d-1)) (pruneTree (right p) (d-1))
    | (d > 1) && isUnerLeft p = makeBinTree (akar p) (pruneTree (left p) (d-1)) Empty
    | (d > 1) && isUnerRight p = makeBinTree (akar p) Empty (pruneTree (right p) (d-1))

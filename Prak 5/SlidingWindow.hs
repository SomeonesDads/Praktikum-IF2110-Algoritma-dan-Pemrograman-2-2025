...
slidingWindow n f xs  
    | isEmpty xs = []
    | (length xs) >= n = (konso (f (takeN n xs)) (slidingWindow n f (tail xs)))
    | otherwise = []
...
-}
windowRange:: [Int] -> Int
windowRange l = (lmax l minBound) - (lmin l maxBound)

lmax:: [Int] -> Int -> Int
lmax l cur_max=
    if isEmpty l then cur_max
    else if cur_max < (head l) then lmax (tail l) (head l)
        else lmax(tail l) cur_max

lmin :: [Int] -> Int -> Int
lmin l cur_max=
    if isEmpty l then cur_max
    else if cur_max > (head l) then lmin (tail l) (head l)
        else lmin(tail l) cur_max

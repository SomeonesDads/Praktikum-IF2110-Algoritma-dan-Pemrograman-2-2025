...
isOrigin :: Point -> Bool
isOrigin a = ordinat a == 0 && absis a == 0
...
kuadran :: Point -> Int
kuadran p 
    | ordinat p > 0 && absis p > 0 = 1
    | ordinat p > 0 && absis p < 0 = 2
    | ordinat p < 0 && absis p < 0 = 3
    | ordinat p < 0 && absis p > 0 = 4
...
jarak a b = 
    let
        dy = abs(ordinat a - ordinat b)
        dx = abs(absis a - absis b)
        in
            sqrt(dx*dx + dy*dy)

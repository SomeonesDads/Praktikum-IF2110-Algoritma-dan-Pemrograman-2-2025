...
import Numeric (floatToDigits, showFloat)
...
isValidMahasiswa m = (nama m /= "" && nim m /= "" && ipk m >= 0 && ipk m <= 4) 
...
gantiIPK :: Mahasiswa -> Float -> Mahasiswa
gantiIPK m i
    | i > 4 = makeMahasiswa (nama m) (nim m) 4.0
    | i < 0 = makeMahasiswa (nama m) (nim m) 0.0
    | i>=0 && i<=4 = makeMahasiswa (nama m) (nim m) i
